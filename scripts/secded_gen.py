...
def gen_code(codetype, k, m, seed=None):
    ...
    random.seed(seed if seed is not None else _RND_SEED)
    try:
...
def write_enc_dec_files(n, k, m, codes, suffix, outdir, codetype, prefix="prim"):
    enc_out = print_enc(n, k, m, codes, codetype)

    module_name = "%s_secded%s_%d_%d" % (prefix, suffix, n, k)
