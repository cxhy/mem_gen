...
def _expand_env_path(path_str: str) -> Path | None:
    """Expand ${VAR} / $VAR in a path string and return a Path.

    Returns None if the path contains unresolved variables after expansion.
    """
...
def _resolve_vendor_path(
    ...
    """Resolve a vendor cell file to its lib_paths-based path.

    Returns the path as configured in lib_paths + lib_name (e.g.
    'vendor/demo/DEMO_SP_256x32.v'), preserving the user's original
    path convention from vendor_port_map.json.

    Supports paths with environment variables (${VAR}, $VAR):
    - If the variable is set, expands and checks file existence
    - If the variable is unset, skips existence check and returns the
      first matching extension (.v preferred over .sv)
    """
    for search_dir in lib_paths:
        resolved = _expand_env_path(search_dir)
        if resolved is not None:
            # Path fully resolved — check file existence
            abs_path = resolved if resolved.is_absolute() else project_root / resolved
            for ext in (".v", ".sv"):
                candidate = abs_path / f"{lib_name}{ext}"
                if candidate.exists():
                    return f"{search_dir}/{lib_name}{ext}".replace("\\", "/")
        else:
            # Path contains unresolved env vars — skip existence check
            return f"{search_dir}/{lib_name}.v".replace("\\", "/")
    return None
...
def _write_filelist(
    ...
    prj_path = project_config.prj_path

    def _prefix(p: str) -> str:
        return f"{prj_path}/{p}" if prj_path else p
    ...
    for mem_spec in project_config.memories:
        vp = _resolve_vendor_path(
            mem_spec.physical.lib_name, lib_paths, project_root,
        )
        if vp is not None:
    ...
    for result in results:
        ...
        for of in result.get("output_files", []):
            ...
            _add(_prefix(rel), instance_entries)
        if instance_entries:
            lines.append("")
